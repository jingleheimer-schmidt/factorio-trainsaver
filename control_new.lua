...
---returns the distance in tiles between two MapPositions
---@param position_1 MapPosition
---@param position_2 MapPosition
---@return number
local function distance_in_tiles(position_1, position_2)
  return math.sqrt( (position_1.x - position_2.x)^2 + (position_1.y - position_2.y)^2 )
end

---creates a single waypoint for a given target
---@param player LuaPlayer
---@param target LuaEntity
---@param start_position MapPosition?
---@return CutsceneWaypoint
local function create_waypoint_to_target(player, target, start_position)
  local mod_settings = player.mod_settings
  local transition_speed = mod_settings["ts-transition-speed"].value * 60000 -- (1000 / 60 * 60 * 60) -- km/h to tiles per tick
  local distance = distance_in_tiles(start_position or player.position, target.position)
  local transition_time = distance / transition_speed
  if transition_time == math.huge then transition_time = 0 end
  local time_to_wait = mod_settings["ts-time-wait"].value * 60 * 60 -- minutes to ticks
  local waypoint = {
    target = target,
    transition_time = transition_time,
    time_to_wait = time_to_wait,
    zoom = mod_settings["ts-zoom"].value,
  }
  return waypoint
end

---creates a single waypoint for a given position
---@param player LuaPlayer
---@param position MapPosition
---@param start_position MapPosition?
---@return CutsceneWaypoint
local function create_waypoint_to_position(player, position, start_position)
  local mod_settings = player.mod_settings
  local time_to_wait = mod_settings["ts-time-wait"].value * 60 * 60 -- minutes to ticks
  local transition_speed = mod_settings["ts-transition-speed"].value * 60000 -- (1000 / 60 * 60 * 60) -- km/h to tiles per tick
  local distance = distance_in_tiles(start_position or player.position, position)
  local transition_time = distance / transition_speed
  if transition_time == math.huge then transition_time = 0 end
  local waypoint = {
    position = position,
...
---create cutscene waypoints for a given target, returning to the player character
---@param player LuaPlayer
---@param target LuaEntity
---@return CutsceneWaypoint[]
local function create_waypoints(player, target)
  local waypoints = {
    create_waypoint_to_target(player, target),
  }
  local waypoint_2_target = player.cutscene_character or player.character
  if waypoint_2_target then
    local waypoint_2_start_position_estimate = target.position
    if target.train then
      if target.train.path_end_stop then
        waypoint_2_start_position_estimate = target.train.path_end_stop.position
      elseif target.train.path_end_rail then
        waypoint_2_start_position_estimate = target.train.path_end_rail.position
      end
    end
    waypoints[2] = create_waypoint_to_target(player, waypoint_2_target, waypoint_2_start_position_estimate)
  end
  return waypoints
end

---start a cutscene for a given player
---@param player LuaPlayer
---@param waypoints CutsceneWaypoint[]
local function create_cutscene(player, waypoints)
...
---train changed state event handler
---@param event EventData.on_train_changed_state
local function train_changed_state(event)
  local train = event.train
  local old_state = event.old_state
  local new_state = train.state
  if not ( (old_state == defines.train_state.wait_station) and ( (new_state == defines.train_state.on_the_path) or (new_state == defines.train_state.arrive_signal) ) ) then return end
  local current_targets = global.current_targets
  if not current_targets then return end
  for _, player in pairs(game.connected_players) do
    if not player.controller_type == defines.controllers.cutscene then goto next_player end
    local current_target = current_targets[player.index]
    if not current_target.valid then goto next_player end
    local current_target_train = current_target.train
    if not current_target_train then goto next_player end
    local current_target_state = current_target_train.state
    if ( (current_target_state == defines.train_state.on_the_path) or (current_target_state == defines.train_state.arrive_signal) or (current_target_state == defines.train_state.arrive_station) ) then
      if current_target_train.id == train.id then
        global.cutscene_next_tick = global.cutscene_next_tick or {}
        global.cutscene_next_tick[player.index] = { train = current_target_train }
      end
    end
    ::next_player::
  end
...
local function on_tick()
  local cutscene_next_tick = global.cutscene_next_tick
  if not cutscene_next_tick then return end
  for player_index, data in pairs(cutscene_next_tick) do
    local player = game.get_player(player_index)
    if not player then goto next_player end
    local train = data.train
    if not train.valid then goto next_player end
    local waypoints = create_waypoints(player, train)
    create_cutscene(player, waypoints)
    ::next_player::
  end
  global.cutscene_next_tick = nil
end
